...
const getCleanNodeData = (data) => {
    // 1. Destructure out all non-serializable properties (functions and derived state)
    // 'level' is derived state and should not be saved permanently.
    const { onUpdate, onDelete, toggleCollapse, selectedTags, level, ...cleanData } = data;

    // 2. ⚠️ CRITICAL FIX: Filter out any remaining properties that are explicitly 'undefined'.
...
const getLayoutedElements = (nodes, edges) => {
  ...
  const layoutedNodes = nodes.map((node) => {
    ...
    // 1. If positionAbsolute exists (manual position/spouse), use it
    if (node.positionAbsolute) {
        // Ensure position is updated to positionAbsolute for ReactFlow to render it correctly
        return {
            ...node,
...
const TreeView = () => {
  ...
  // CRITICAL: Get the treeId from the URL parameters
  ...
  const [user, setUser] = useState(auth.currentUser); // CRITICAL FIX: Initialize with current user
  ...
  // Handle saving the current state to Firestore (CALLED WITH treeId)
  const handleSave = useCallback(async (currentNodes, currentEdges) => {
      if (!user || !treeId) {
          console.warn("Save skipped: User or Tree ID missing.");
          return;
      }
      ...
      try {
        const cleanNodes = currentNodes.map((n) => ({
          id: n.id,
          type: n.type,
          // CRITICAL FIX: Explicitly check for position before inclusion to avoid 'undefined'
          ...(n.position && { position: n.position }),
          // Only include positionAbsolute if it's defined to avoid 'undefined' error
        ...
        // CRITICAL: Pass treeId to save function
  ...
  const takeSnapshot = useCallback((currentNodes, currentEdges) => {
    // 1. Clean data for history/Firestore (this is mostly for serialization safety)
    const snapshotNodes = currentNodes.map((n) => ({
        id: n.id, type: n.type, 
        ...(n.position && { position: n.position }),
        // FIREBASE FIX: Only include positionAbsolute if it's defined to avoid 'undefined' error
        ...(n.positionAbsolute && { positionAbsolute: n.positionAbsolute }), 
        data: getCleanNodeData(n.data),
    }));
    const snapshotEdges = currentEdges.map((e) => ({
        id: e.id, source: e.source, target: e.target, type: e.type,
        ...(e.style && { style: e.style }), 
        ...(e.markerEnd && { markerEnd: e.markerEnd }),
    }));
    ...
    setHistory(h => {
        ...
        return [...newHistory, { nodes: snapshotNodes, edges: snapshotEdges }];
    });
  ...
  // CRITICAL FIX: Custom onNodesChange to handle drag events and trigger snapshot
  const onNodesChange = useCallback((changes) => {
    onNodesChangeReactFlow(changes);
    
    // Check for a finish-drag event
    const dragEndChange = changes.find(change => change.type === 'position' && change.position);

    if (dragEndChange) {
        const nodeId = dragEndChange.id;
        const newPosition = dragEndChange.position;

        // Use a timeout to ensure ReactFlow updates its position state internally before we read it
        setTimeout(() => {
            setNodes(nds => {
                const nodesAfterDrag = nds.map(n => {
                    if (n.id === nodeId) {
                        // CRITICAL: For a manual drag, save the position to positionAbsolute 
                        // so Dagre knows not to move it later.
                        return { ...n, positionAbsolute: { x: newPosition.x, y: newPosition.y } };
                    }
                    return n;
                });
                
                // Trigger snapshot/save with the nodes containing positionAbsolute
                takeSnapshot(nodesAfterDrag, edges);
                return nodesAfterDrag;
            });
        }, 10);
    }

  }, [onNodesChangeReactFlow, takeSnapshot, setNodes, edges]);
  ...
  const applyLayoutAndLevels = useCallback((updatedNodes, updatedEdges, runLayout = true, skipSnapshot = false) => {
      ...
      // 2. Inject Callbacks and Levels
      const nodesWithCallbacks = finalNodes.map((node) => ({
          ...node,
          data: {
              // Ensure only clean data is used for merging to prevent 'undefined' issues
  ...
  useEffect(() => {
    // Set up a listener for auth changes (used for navigation/logout)
    const unsubscribe = auth.onAuthStateChanged((currentUser) => {
        setUser(currentUser);
        if (!currentUser) {
            // If user logs out, go back to home screen (tree selector/login)
            navigate('/');
        } else if (treeId) {
            // User is authenticated, proceed to load the specific tree
            loadTree(treeId); 
        }
    });
    ...
    return () => unsubscribe();
  }, [navigate, treeId]); // Depend on navigate and treeId

  const loadTree = async (currentTreeId) => {
    setLoading(true);
    try {
      // CRITICAL: Pass treeId to load function
  ...
  const createInitialTree = () => {
    // Use the treeId (surname) in the label
    const rootNode = {
      id: 'root_' + Date.now(),
      type: 'customNode',
      data: {
  ...
  const addNode = useCallback(
    (type) => {
      ...
      if (type === 'child') {
        ...
        // Calculate position next to the selected node and use positionAbsolute.
  ...
  // Function to handle multi-select tag filter
  const handleTagToggle = (tag) => {
    if (tag === '') {
        // Selecting 'All Tags' clears all filters
        setSelectedTags([]);
    } else {
  ...
  // Function to manually trigger the layout
  const handleApplyLayout = useCallback(() => {
    // Re-apply layout to the current state (nodes and edges).
    // Setting runLayout=true and skipSnapshot=false
